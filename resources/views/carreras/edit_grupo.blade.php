...
<head>
    ...
    <title>{{ __('Editar Grupo') }} | {{ $grupo->Grupo }}</title>
...
<body>
        ...
        <main class="main-content">
            <header class="student-header">
                <div class="student-profile">
                    ...
                    <div class="student-info">
                        <h1>{{ __('Editar Grupo') }}: {{ $grupo->Grupo }}</h1>
                        <p class="student-id">{{ __('Actualización del catálogo de grupos') }}</p>
                    </div>
                </div>
            </header>
            ...
            <div class="dashboard-grid">
                <div class="card full-width">
                    <h3><i class="fa-solid fa-pen-to-square"></i> {{ __('Modificar Datos del Grupo') }}</h3>

                    <form id="edit-grupo-form" action="{{ route('grupos.update', $grupo->idGrupos) }}" method="POST">
                        @csrf
                        @method('PATCH') 

                        <div class="form-grid">
                            <div class="form-group">
                                <label>{{ __('Nombre del Grupo') }}</label>
                                <input type="text" name="Grupo" class="form-control" value="{{ $grupo->Grupo }}" required>
                            </div>

                            <div class="form-group">
                                <label>{{ __('Capacidad / Cantidad de Alumnos') }}</label>
                                <input type="number" name="Cantidad_Alumnos" class="form-control" value="{{ $grupo->Cantidad_Alumnos }}" required>
                            </div>

                            <div class="form-group" style="grid-column: 1 / -1;">
                                <label>{{ __('Tutor Asignado') }}</label>
                                <select name="idTutores" class="form-control">
                                    <option value="">{{ __('Sin Tutor Asignado') }}</option>
                                    @foreach($tutores as $tutor)
                                        <option value="{{ $tutor->idTutores }}" {{ $grupo->idTutores == $tutor->idTutores ? 'selected' : '' }}>
                                            {{ $tutor->Nombre }} {{ $tutor->Apellido }} | {{ $tutor->Clave_Trabajador }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-actions">
                                <a href="{{ route('materias.show', $grupo->idCarreras) }}" class="btn-secondary" style="text-decoration: none; display: flex; align-items: center; justify-content: center;">{{ __('Cancelar') }}</a>
                                <button type="submit" class="btn-primary">
                                    <i class="fa-solid fa-rotate"></i> {{ __('Actualizar Grupo') }}
                                </button>
                            </div>
                        </div>
                    </form>
    ...
    <script>
        ...
        document.getElementById('edit-grupo-form').addEventListener('submit', async function(e) {
            e.preventDefault();
            const form = this;
            ...
            document.addEventListener('DOMContentLoaded', async () => {
                ...
                if (ok) {
                    Swal.fire({
                        ...
                        timer: 1500,
                        showConfirmButton: false,
                        willClose: () => document.getElementById('edit-grupo-form').submit()
                    });
                }
            });
        @endif

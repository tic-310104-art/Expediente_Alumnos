...
<head>
    ...
    <title>{{ __('Mis Citas de Tutoría') }} (PDF)</title>
    ...
    <div class="page">
        <div class="header">
            <div class="university-info">
                <h1>{{ __('Universidad Tecnológica de Nayarit') }}</h1>
                <p>{{ __('Dirección de Servicios Escolares') }}</p>
            </div>
        ...
        <div class="info-grid">
            ...
            <div class="info-item">
                <label>{{ __('Tutor') }}</label>
                <span>{{ $cita->tutor ? ($cita->tutor->Nombre . ' ' . $cita->tutor->Apellido) : 'No asignado' }}</span>
            </div>
            <div class="info-item">
                <label>{{ __('Carrera') }}</label>
                <span>{{ $alumno->carrera->Nombre ?? __('Sin Carrera') }}</span>
            </div>
        </div>
        ...
        <section>
            <table>
                ...
                <tbody>
                    <tr>
                        <td>{{ \Carbon\Carbon::parse($cita->Fecha)->format('d/m/Y h:i A') }}</td>
                        <td>{{ $cita->Motivo }}</td>
                        <td>{{ \Carbon\Carbon::parse($cita->Fecha)->isPast() ? __('Realizada') : __('Programada') }}</td>
                    </tr>
                </tbody>
            </table>
        </section>
        ...
        <div class="footer-sign">
            <div class="sign-box">
                <div class="sign-line">{{ __('Firma del Tutor') }}</div>
            </div>
            <div class="sign-box">
                <div class="sign-line">{{ __('Firma del Alumno') }}</div>
            </div>
        </div>
    </div>
